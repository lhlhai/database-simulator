...
function App() {
  ...
  const run = async (nextDialect = dialect, nextQuery = query) => {
    ...
    } catch (e) { setError(e instanceof Error ? e.message : 'Không thể mô phỏng query này.'); setWorkbenchTab('messages') }
  }
  const compareQueries = async () => {
    ...
    catch (e) { setError(e instanceof Error ? e.message : 'Không thể chạy comparison.'); setWorkbenchTab('messages') }
  }
...
function DataStudio({ tables, onChange }: { tables: UserTable[]; onChange: (tables: UserTable[]) => void }) {
  const [activeTable, setActiveTable] = useState(0)
  const table = tables[activeTable] ?? tables[0]
  if (!table) return <div className="studio-empty">No tables loaded. Upload CSV/TXT to start editing.</div>
  const updateCell = (rowIndex: number, column: string, value: string) => {
    const next = tables.map((item, index) => index === activeTable ? { ...item, rows: item.rows.map((row, index) => index === rowIndex ? { ...row, [column]: value } : row) } : item)
    onChange(next)
  }
  return <div className="data-studio"><aside className="table-rail"><div className="rail-title"><span>SCHEMA</span><strong>{tables.length} tables</strong></div>{tables.map((item, index) => <button key={item.name} className={index === activeTable ? 'selected' : ''} onClick={() => setActiveTable(index)}><Database size={13} /><span><strong>{item.name}</strong><small>{item.rows.length} rows · {item.columns.length} cols</small></span></button>)}</aside><div className="table-editor"><div className="table-editor-head"><div><span className="section-kicker">DATA EDITOR</span><h3>{table.name}</h3><small>{table.source} · click any cell to edit</small></div><span className="table-badge">{table.rows.length} × {table.columns.length}</span></div><div className="table-scroll"><table><thead><tr><th>#</th>{table.columns.map((column) => <th key={column}>{column}<small>TEXT / VALUE</small></th>)}</tr></thead><tbody>{table.rows.map((row, rowIndex) => <tr key={rowIndex}><td className="row-number">{rowIndex + 1}</td>{table.columns.map((column) => <td key={column}><input aria-label={`${table.name} row ${rowIndex + 1} ${column}`} value={String(row[column] ?? '')} onChange={(event) => updateCell(rowIndex, column, event.target.value)} /></td>)}</tr>)}</tbody></table></div></div></div>
}

function MessagesPanel({ error, query, plan }: { error: string; query: string; plan: QueryPlan }) {
  return <div className="messages-panel"><div className="console-head"><div><span className="section-kicker">MESSAGES / CONSOLE</span><h3>{error ? 'Execution failed' : 'Execution diagnostics'}</h3></div><span className={error ? 'console-status error' : 'console-status'}>{error ? 'ERROR' : 'READY'}</span></div>{error ? <div className="console-error"><X size={16} /><div><strong>{error}</strong><p>Fix the query or dataset, then run it again. The current plan was not replaced.</p></div></div> : <div className="console-success"><ShieldCheck size={16} /><div><strong>Query is ready to visualize</strong><p>{plan.nodes.length} operators · {plan.metrics.returned} result rows · strategy: {plan.metrics.strategy}</p></div></div>}<div className="console-log"><span className="log-prefix">QUERY</span><code>{query}</code><span className="log-prefix">PLAN</span><code>{plan.nodes.map((node) => node.label).join(' → ')}</code><span className="log-prefix">NOTE</span><p>Educational execution model. Metrics explain the selected plan; they are not a production benchmark.</p></div></div>
}
