...
function MinePlayBoard({ state, rows, plan, onAction, onReset, autoEnabled = false }: { state: PlayState; rows: RowData[]; plan: QueryPlan; onAction: (action: PlayAction) => void; onReset: () => void; autoEnabled?: boolean }) {
  ...
  return <div className="mine-game">
    ...
    <div className="mine-layout"><section className="mine-shaft"><div className="shaft-label"><span>DATA MINE</span><small>{state.complete ? 'ALL TREASURE EXCAVATED' : current ? `NUGGET ${state.cursor + 1} / ${rows.length}` : 'READY TO DIG'}</small></div><div className="ore-wall">{rows.map((row, index) => { const id = String(row.id ?? index); const mined = state.resultIds.includes(id) || state.filteredIds.includes(id); const active = current && id === String(current.id ?? state.cursor) && !state.complete; return <div className={`ore-nugget ${mined ? 'mined' : ''} ${active ? 'dig-target' : ''}`} key={id}><span className="ore-glint"><Gem size={16} /></span><div><strong>{String(row.name ?? `row_${id}`)}</strong><small>{id} · {String(row.age ?? '—')} · {String(row.city ?? row.role ?? 'data')}</small></div>{active && <span className="dig-label"><Pickaxe size={11} /> DIG</span>}{mined && <ShieldCheck size={15} className="mined-mark" />}</div> })}</div><div className="miner-line"><Pickaxe size={17} /><span>Mining one record at a time keeps the invisible execution visible.</span></div></section><section className="mine-actions"><div className="station-title"><span>PROCESSING STATION</span><small>What should the miner do?</small></div><button className="mine-action treasure" disabled={!current || state.complete} onClick={() => onAction('result')}><span className="action-orb"><Trophy size={18} /></span><span><strong>Send to treasure chest</strong><small>{isFilterGame ? 'Passes the WHERE gate' : 'Keep this row in result'}</small></span><ArrowDown size={15} /></button>{isFilterGame && <button className="mine-action chute" disabled={!current || state.complete} onClick={() => onAction('filter')}><span className="action-orb"><ArrowDown size={18} /></span><span><strong>Drop into filter chute</strong><small>Does not pass the predicate</small></span><X size={15} /></button>}<button className="mine-auto" onClick={() => setAutoPlaying((value) => !value)}>{autoPlaying ? <Pause size={14} /> : <Play size={14} fill="currentColor" />} {autoPlaying ? 'Pause auto-miner' : 'Start auto-miner'}</button><button className="mine-reset" onClick={() => { setAutoPlaying(false); onReset() }}><RotateCcw size={13} /> Reset mine</button></section><section className="treasure-chest"><div className="chest-heading"><Trophy size={16} /><div><strong>RESULT CHEST</strong><small>{completedRows.length} treasure{completedRows.length === 1 ? '' : 's'} collected</small></div></div><div className="chest-body">{completedRows.length === 0 ? <div className="chest-empty"><Gem size={22} /><span>Empty chest</span><small>Correct rows land here</small></div> : completedRows.map((row, index) => <div className="chest-item" key={String(row.id ?? index)}><Gem size={14} /><span><strong>{String(row.name ?? `row_${row.id}`)}</strong><small>{String(row.id ?? index)} · {String(row.age ?? '—')}</small></span><ShieldCheck size={14} /></div>)}</div><div className="filter-pit"><span>FILTER PIT</span>{filteredRows.length ? filteredRows.map((row, index) => <b key={String(row.id ?? index)}><X size={10} /> {String(row.name ?? row.id)}</b>) : <small>nothing discarded yet</small>}</div></section></div>
    {state.complete && <div className="mine-complete"><Trophy size={18} /><div><strong>Mine cleared!</strong><span>{completedRows.length} treasure{completedRows.length === 1 ? '' : 's'} recovered · {filteredRows.length} filtered out</span></div><button onClick={() => { setAutoPlaying(false); onReset() }}>Play again</button></div>}
  </div>
}
