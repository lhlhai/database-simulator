...
import { createPlayState, isPlaySupported, takePlayAction, type PlayAction, type PlayState } from './lib/playMode'
import './styles.css'
...
function App() {
  ...
  const run = (nextDialect = dialect, nextQuery = query) => {
    try { setError(''); setPlaying(false); setPlayState(createPlayState()); setPlan(simulate(nextDialect, nextQuery, users)); setStep(-1) }
  ...
  const playAction = (action: PlayAction) => setPlayState((state) => takePlayAction(plan, state, users, action))
