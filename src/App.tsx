...
function App() {
  ...
  const [viewMode, setViewMode] = useState<'watch' | 'play'>('watch')
  ...
  return <div className="app-shell">
    ...
    <main className="workspace">
      ...
      {compareOpen && <ComparePanel dialect={dialect} queryA={compareA} queryB={compareB} setQueryA={setCompareA} setQueryB={setCompareB} comparison={comparison} onRun={compareQueries} />}
...
function ComparePanel({ dialect, queryA, queryB, setQueryA, setQueryB, comparison, onRun }: { dialect: Dialect; queryA: string; queryB: string; setQueryA: (value: string) => void; setQueryB: (value: string) => void; comparison: [ComparisonResult, ComparisonResult] | null; onRun: () => void }) {
  return <section className="compare-panel panel"><div className="compare-head"><div><span className="section-kicker">04 / SIDE-BY-SIDE EXPLAINER</span><h2>Compare two query strategies</h2><p>Chạy cùng dataset trong cùng browser session. Elapsed time chỉ là chỉ báo client-side, còn metrics giải thích execution path.</p></div><button className="run-button" onClick={onRun}><Zap size={14} /> Run both</button></div><div className="compare-editors"><label><span>QUERY A</span><textarea value={queryA} onChange={(event) => setQueryA(event.target.value)} spellCheck={false} /></label><label><span>QUERY B</span><textarea value={queryB} onChange={(event) => setQueryB(event.target.value)} spellCheck={false} /></label></div>{comparison ? <div className="compare-results">{comparison.map((item, index) => <div className="compare-card" key={index}><div className="compare-card-title"><span>QUERY {index === 0 ? 'A' : 'B'}</span><strong>{item.plan.metrics.strategy}</strong><b>{item.elapsedMs} ms</b></div><div className="compare-metrics"><span>scanned <b>{item.plan.metrics.scanned}</b></span><span>matched <b>{item.plan.metrics.matched}</b></span><span>returned <b>{item.plan.metrics.returned}</b></span></div><p>{item.plan.explanation}</p></div>)}</div> : <div className="compare-empty">Chưa chạy comparison. Chọn hai query trên cùng dataset rồi bấm Run both.</div>}<small className="compare-disclaimer">Dialect: {dialect.toUpperCase()} · tốc độ phụ thuộc kích thước dataset, browser và thời điểm chạy; không phải benchmark production.</small></section>
}
